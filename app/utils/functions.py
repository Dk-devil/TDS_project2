...
def generate_duckdb_query(
    ...
    Args:
        query_type (str): Type of query to generate (e.g., 'post_comments', 'user_activity').
        timestamp_filter (str, optional): ISO timestamp for filtering data (e.g., '2025-02-26T00:17:09.465Z').
        numeric_filter (int, optional): Numeric threshold for filtering (e.g., 5 for star count).
        sort_order (str, optional): Sort order for results ('ASC' or 'DESC'). Default is 'DESC'.

    Returns:
        str: Generated DuckDB SQL query.
    """

    # Define base queries for different query types
    queries = {
        "post_comments": "SELECT * FROM post_comments",
    ...
    if numeric_filter is not None:
        conditions.append(f"numeric_value >= {numeric_filter}")
...
# Example usage
query = generate_duckdb_query(
    query_type="post_comments",
    timestamp_filter="2025-02-26T00:17:09.465Z",
    numeric_filter=5,
    sort_order="ASC"
)
print(query)  # Output: SELECT * FROM post_comments WHERE timestamp >= '2025-02-26T00:17:09.465Z' AND numeric_value >= 5 ORDER BY timestamp ASC

async def transcribe_youtube_segment(youtube_url: str, start_time: float, end_time: float) -> str:
    """
    Extract audio from a YouTube video for a specific segment and transcribe it.
    ...
    try:
        ...
        if not audio_stream:
            return "Error: No audio stream found in the video."
        ...
        clip = AudioFileClip(audio_file_path).subclip(start_time, end_time)
        segment_path = os.path.join(temp_dir, "segment.wav")
        clip.write_audiofile(segment_path, codec="pcm_s16le")
        clip.close()

        # Step 3: Transcribe the extracted segment
